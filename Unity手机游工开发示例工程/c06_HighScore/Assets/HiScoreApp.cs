...
public class HiScoreApp : MonoBehaviour {
    ...
    IEnumerator UploadScore(string name, string score)
    {
        ...
        if (useRedis)
            url = "http://127.0.0.1/uploadscore_redis.php";
        else
            url = "http://127.0.0.1/uploadscore.php";
    ...
    IEnumerator DownloadScores(string name, string score)
    {
        string url = string.Empty;
        if (useRedis)
            url = "http://127.0.0.1/downloadscores_redis.php";
        else
            url = "http://127.0.0.1/downloadscores.php";
        WWW www = new WWW(url);
